...
#### 1. Tree-sitter 集成 (高优先级)

```python
# 目标接口
class TreeSitterBackend:
    def parse_file(self, filepath: str) -> AST
    def extract_functions(self, ast: AST) -> List[Function]
    def extract_structs(self, ast: AST) -> List[Struct]
    def extract_calls(self, ast: AST, function: Function) -> List[Call]
```

**任务分解:**
- [ ] 集成 tree-sitter Python 绑定
- [ ] 编写 C 语言查询 (S-expression)
- [ ] 实现函数提取
- [ ] 实现结构体提取
- [ ] 实现调用提取
- [ ] 处理预处理器指令
...
- [ ] **M2**: tree-sitter 集成 (目标: 2024-Q2)
- [ ] **M3**: libclang 集成 (目标: 2024-Q3)
- [ ] **M4**: 跨文件分析 (目标: 2024-Q4)
- [ ] **M5**: VSCode 扩展 Beta (目标: 2025-Q1)
- [ ] **M6**: v1.0 正式发布 (目标: 2025-Q2)
